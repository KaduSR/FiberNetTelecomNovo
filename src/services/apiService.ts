...
import { DashboardResponse, LoginResponse, BoletoSearchResponse, OrdemServico, TicketType, TicketCreatePayload } from '../types/api';
...
class ApiService {
  ...
  async changePassword(newPassword: string, oldPassword?: string): Promise<{ message: string }> {
    return this.request<{ message: string }>(ENDPOINTS.CHANGE_PASSWORD, {
      method: 'POST',
      body: JSON.stringify({ 
        novaSenha: newPassword,
        senhaAtual: oldPassword // Optional based on guide, but usually required
      }),
    });
  }

  async validatePassword(senha: string): Promise<any> {
    return this.request(ENDPOINTS.VALIDATE_PASSWORD, {
      method: 'POST',
  ...
  async searchInvoicesByCpf(cpfCnpj: string): Promise<BoletoSearchResponse> {
    // Remove formatação conforme guia
    const cpfLimpo = cpfCnpj.replace(/\D/g, '');
    
    return this.request<BoletoSearchResponse>(ENDPOINTS.SEARCH_INVOICES_CPF, {
      method: 'POST',
      body: JSON.stringify({ cpfCnpj: cpfLimpo })
    });
  }

  async getServiceOrders(): Promise<OrdemServico[]> {
    const data = await this.request<any>(ENDPOINTS.SERVICE_ORDERS);
    return data.ordens || [];
  }

  async getTicketTypes(): Promise<{ tipos: TicketType[], prioridades: any[] }> {
    return this.request<any>(ENDPOINTS.TICKET_TYPES);
  }

  async createTicket(payload: TicketCreatePayload): Promise<any> {
